...
def get_model(line):
    split = line.split(' ')
    return Model(*tuple(map(int, split[0].split('-'))), split[1].split(':')[0], split[2])


def parse(_input):
    return [get_model(e) for e in _input.split("\n") if len(e) > 0]


# solution
def validate1(model):
    return model.min <= model.password.count(model.rule) <= model.max


def validate2(model):
    return model.password[model.min - 1] == model.rule and model.password[model.max - 1] != model.rule \
           or model.password[model.min - 1] != model.rule and model.password[model.max - 1] == model.rule


def count_valid_passwords(passwords, validator) -> int:
    i = 0
    n = len(passwords)
    counter = 0
    while i < n:
        if validator(passwords[i]):
            counter += 1
        i += 1
    return counter
...
# test 1
part_one = count_valid_passwords(lines, validate1)
assert part_one == 2

# test 2

# solve
part_one = count_valid_passwords(parse(read_data()), validate1)
print(part_one)
assert part_one == 603

part_two = count_valid_passwords(parse(read_data()), validate2)
print(part_two)
assert part_two == 404
