...
def parse(_input: str):
    return [get_passport(l) for l in _input.split("\n\n") if len(l) > 0]
...
def count_valid_passports(passports: list, validator) -> int:
    n, i, counter = len(passports), 0, 0

    while i < n:
        if validator(passports[i]):
            counter += 1
        i += 1
...
# test 1
part_one = count_valid_passports(lines, fields_exist)
assert part_one == 2
...
part_two = count_valid_passports(lines, validate_fields)
assert part_two == 4
# SOLUTION
passes = parse(read_data())
# part#1
part_one = count_valid_passports(passes, fields_exist)
print(part_one)
assert part_one == 182
# # part#2
part_two = count_valid_passports(passes, validate_fields)
print(part_two)
assert part_two == 109
