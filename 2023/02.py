...
# MAIN FUNCTIONS
bag1 = {"red": 12, "green": 13, "blue": 14}  # rgb cubes


def part1(given: List[str]) -> int:
    """good ol` nested for loops"""
    result = 0
    for i, game in enumerate(given):
        for turn in game.split(': ')[1].split('; '):
            for cube in turn.split(', '):
                count, color = cube.split(' ')
                if bag1.get(color) < int(count):
                    break
            else:
                continue
            break
...
def part1_comp(given: List[str]) -> int:
    """comprehension unreadable as hell!!!"""
    return sum(i + 1 for i, game in enumerate(given)
               if not any(
        any(bag1.get(color, 0) < int(count)
...
""".strip())
    assert part1(given) == 8
    assert part1_comp(given) == 8
    # part 2
    assert part2(given) == 2286
    # assert part2_comp(given) == 2286
